...
const AllocateClassToStudent = () => {
  ...
  const fnFetchAllocatedClassroom = async (StudentID) => {
    setLoad(true);
    const { data, status } = await callApi({ url: `/api/students/${StudentID}/classrooms`, method: 'GET' });
    setLoad(false);
    if(status){
      setDatasetGrid(data);
    } else{
  ...
  const fnValidateData = () => {
    if(objInput.StudentID === "default") return { isValid: false, err: "Please select students." }
  ...
  const fnSave = async () => {
    ...
    const { data, status } = await callApi({
      url: `/api/students/${objInput.StudentID}/classrooms`,
      method: "POST",
      data: arrClassrooms,
    });
    setLoad(false);
              ...
              <div className="row">
                <div className="col-6">
                  <FormGroup>
                    ...
                    <Input id="StudentID" name="StudentID" type="select" onChange={fnOnChangeStudent}>
                      ...
                      {arrStudents.map((val) => {
                        return (
                          <>
                            <option value={val.studentID}>{val.firstName}{" "}{val.lastName}</option>
                          </>
                        );
                      })}
                    </Input>
...
export default AllocateClassToStudent;